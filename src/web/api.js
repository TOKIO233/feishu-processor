...
const FEISHU_BINARY_NAME = process.platform === 'win32' ? 'feishu2md.exe' : 'feishu2md';
...
function executeFeishuDownload(url, downloadsDir) {
  return new Promise((resolve, reject) => {
    const feishuCommand = resolveFeishuCommand();
    ...
    childProcess.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error(buildFeishuErrorMessage(code, stderr)));
      }
    });

    childProcess.on('error', (error) => {
      reject(new Error(
        `启动feishu2md失败: ${error.message}。请先执行 scripts/install-feishu2md.sh，或设置 FEISHU2MD_BIN。`
      ));
    });
  });
}

function resolveFeishuCommand() {
  const candidates = [
    process.env.FEISHU2MD_BIN,
    path.join(process.cwd(), 'tools', 'feishu2md', 'current', FEISHU_BINARY_NAME),
    path.join(process.cwd(), 'tools', 'feishu2md', FEISHU_BINARY_NAME)
  ].filter(Boolean);
  ...
  return FEISHU_BINARY_NAME;
}

function buildFeishuEnv() {
  const bundledDir = path.join(process.cwd(), 'tools', 'feishu2md', 'current');
  const currentPath = process.env.PATH || '';

  return {
    ...process.env,
    PATH: `${bundledDir}${path.delimiter}${currentPath}`
  };
}

function buildFeishuErrorMessage(code, stderr) {
  const output = (stderr || '').trim();
  const baseMessage = `feishu2md执行失败，退出码: ${code}, 错误: ${output || '无错误输出'}`;

  if (output.includes('config.json: no such file or directory')) {
    return `${baseMessage}。请先配置 feishu2md：` +
      './tools/feishu2md/current/feishu2md config --appId <id> --appSecret <secret>。';
  }
...
router.get('/download-file', (req, res) => {
  const filePath = normalizeInputPath(req.query.path);
...
router.post('/read-file', async (req, res) => {
  const filePath = normalizeInputPath(req.body.filePath);
